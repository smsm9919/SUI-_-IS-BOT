...
• SMART MONEY HYBRID ENGINE - نظام SMC + سيولة + بنية السوق
• DECISION MATRIX ENGINE - نظام تصويت ذكي لمنع التداول العشوائي
• SMART TRAILING & TP LADDER - جني أرباح ذكي حسب نوع الترند
"""
...
# =========================
# SMART MONEY ENGINE IMPORTS
# =========================

# سيتم إضافة المحركات الجديدة في نفس الملف لتجنب تعقيدات الاستيراد
...
# =========================
# SMART MONEY HYBRID ENGINE (SMHE)
# =========================
...
class SmartMoneyHybridEngine:
    """محرك Smart Money Hybrid (SMC + Liquidity + Structure)"""
    ...
    def __init__(self, candles: List[Dict[str, float]]):
        ...
        self.volume = np.array([c["volume"] for c in candles])
    ...
    def market_state(self) -> MarketState:
        if len(self.close) < 20:
            return MarketState("NO_TRADE", 0.0, "NONE")
        
        adx_proxy = np.mean(np.abs(np.diff(self.close[-20:])))
        trend_strength = adx_proxy * 100

        if trend_strength < 0.15:
            return MarketState("NO_TRADE", trend_strength, "NONE")

        direction = "BULL" if self.close[-1] > self.close[-10] else "BEAR"
        regime = "TREND" if trend_strength > 0.3 else "RANGE"

        return MarketState(regime, trend_strength, direction)
    
    # =========================
    # LIQUIDITY ENGINE
    # =========================
    def liquidity_state(self) -> LiquidityState:
        if len(self.high) < 20:
            return LiquidityState(False, False, None)
        
        recent_high = max(self.high[-20:-1])
        recent_low = min(self.low[-20:-1])

        swept_high = self.high[-1] > recent_high and self.close[-1] < recent_high
        swept_low = self.low[-1] < recent_low and self.close[-1] > recent_low

        sweep_price = recent_high if swept_high else recent_low if swept_low else None

        return LiquidityState(swept_high, swept_low, sweep_price)
    ...
    def structure_state(self) -> StructureState:
        if len(self.high) < 10:
            return StructureState(False, False, "NONE")
        
        prev_high = max(self.high[-10:-2])
        prev_low = min(self.low[-10:-2])

        bos_bull = self.close[-1] > prev_high
        bos_bear = self.close[-1] < prev_low

        choch_bull = self.close[-1] > prev_high and self.close[-2] < prev_low
        choch_bear = self.close[-1] < prev_low and self.close[-2] > prev_high

        if bos_bull:
            return StructureState(True, False, "BULL")
        if bos_bear:
            return StructureState(True, False, "BEAR")
        if choch_bull:
            return StructureState(False, True, "BULL")
        if choch_bear:
            return StructureState(False, True, "BEAR")
    ...
    # =========================
    def explosion_state(self, atr: float) -> str:
        if len(self.close) < 20:
            return "NORMAL"
        
        body = abs(self.close[-1] - self.open[-1])
        avg_body = np.mean(abs(self.close[-10:] - self.open[-10:]))

        vol_spike = self.volume[-1] > np.mean(self.volume[-20:]) * 1.8

        if body > avg_body * 2 and vol_spike:
            direction = "UP" if self.close[-1] > self.open[-1] else "DOWN"
            return f"EXPLOSION_{direction}"

        if body > avg_body * 2 and not vol_spike:
            return "FAKE_MOVE"
    ...
    # =========================
    def detect_order_block(self) -> Optional[OrderBlock]:
        if len(self.close) < 6:
            return None
    ...
    # FVG DETECTION
    # =========================
    def detect_fvg(self) -> Optional[FVGZone]:
        if len(self.high) < 4:
            return None
        ...
        if h1 < l3:  # Bullish FVG
            return FVGZone(high=l3, low=h1, filled=False)

        l1 = self.low[-3]
        h3 = self.high[-1]

        if l1 > h3:  # Bearish FVG
            return FVGZone(high=l1, low=h3, filled=False)
    ...
    # LIQUIDITY LOG ADDON
    # =========================
    def liquidity_snapshot(self) -> str:
        """لوج السيولة التوضيحي"""
        if len(self.high) < 10:
            return "…"
        ...
        # Sweep High
        if self.high[-1] > max(self.high[-10:-1]) and self.close[-1] < self.high[-2]:
            flags.append("🩸 SELL-SWEEP")
        
        # Sweep Low
        if self.low[-1] < min(self.low[-10:-1]) and self.close[-1] > self.low[-2]:
            flags.append("💧 BUY-SWEEP")
        
        # Accumulation/Distribution
        body = abs(self.close[-1] - self.open[-1])
        rng = self.high[-1] - self.low[-1]
        vol_spike = self.volume[-1] > np.mean(self.volume[-20:]) * 1.5 if len(self.volume) >= 20 else False
        
        if body < rng * 0.3 and vol_spike:
    ...
    # =========================
    def detect_fake_breakout(self, atr: float) -> List[str]:
        """كشف الكسور الوهمية"""
        if len(self.high) < 10:
            return []
        ...
        # حجم الشمعة بالنسبة لـ ATR
        candle_size = self.high[-1] - self.low[-1]
        
        # كسر وهمي للأعلى
        broke_high = self.high[-1] > max(self.high[-10:-1])
        weak_close_up = self.close[-1] < self.high[-2]
        long_wick_up = (self.high[-1] - max(self.close[-1], self.open[-1])) / candle_size > 0.5
        
        if broke_high and weak_close_up and long_wick_up:
            flags.append("FAKE-UP")
        
        # كسر وهمي للأسفل
        broke_low = self.low[-1] < min(self.low[-10:-1])
        weak_close_down = self.close[-1] > self.low[-2]
        long_wick_down = (min(self.close[-1], self.open[-1]) - self.low[-1]) / candle_size > 0.5
        
        if broke_low and weak_close_down and long_wick_down:
            flags.append("FAKE-DOWN")
        
        # عدم الاستمرار
        if flags and abs(self.close[-1] - self.open[-1]) < abs(self.close[-2] - self.open[-2]) * 0.6:
...
class TrendClassifier:
    """مصنف الترند (MID vs LARGE vs CHOP)"""
    
    def __init__(self, candles: List[Dict[str, float]], adx: float, di_plus: float, di_minus: float, volume: List[float]):
        self.high = np.array([c["high"] for c in candles])
        self.low = np.array([c["low"] for c in candles])
        self.close = np.array([c["close"] for c in candles])
        self.adx = adx
    ...
    def structure_expansion(self):
        ...
        return last_range > avg_range * 1.3
    ...
    def classify(self):
        ...
        if (
            self.adx > 30
            and self.structure_expansion()
            and self.volume_expansion()
        ):
            return "LARGE"
...
class TPLadderEngine:
    """محرك جني الأرباح الذكي (TP Ladder)"""
    
    def __init__(self, candles, side, trend_type):
        self.high = np.array([c["high"] for c in candles])
        self.low = np.array([c["low"] for c in candles])
        self.close = np.array([c["close"] for c in candles])
        self.side = side          # BUY / SELL
        self.trend = trend_type  # MID / LARGE
    ...
    def detect_targets(self):
        ...
        if self.side == "BUY":
            if len(self.high) >= 30:
                ...
                targets.append(max(swing_highs))  # Liquidity high

            if self.trend == "LARGE" and len(self.high) >= 60:
                targets.append(max(self.high[-60:]))
        ...
        if self.side == "SELL":
            ...
            if self.trend == "LARGE" and len(self.low) >= 60:
                targets.append(min(self.low[-60:]))
...
class SmartTrailingEngine:
    """محرك الوقف المتحرك الذكي"""
    
    def __init__(self, candles, side):
        self.high = np.array([c["high"] for c in candles])
        self.low = np.array([c["low"] for c in candles])
        self.close = np.array([c["close"] for c in candles])
        self.side = side  # BUY / SELL
    
    def trailing_stop(self, entry_price, current_sl):
        if len(self.low) < 5 or len(self.high) < 5:
            return current_sl

        if self.side == "BUY":
            recent_hl = min(self.low[-5:])
            if recent_hl > current_sl:
                return recent_hl
            return current_sl

        if self.side == "SELL":
            recent_lh = max(self.high[-5:])
            if recent_lh < current_sl:
                return recent_lh
            return current_sl
...
class DecisionMatrixEngine:
    ...
    def add_vote(self, side: str, weight: float = 1.0, reason: str = ""):
        """إضافة تصويت للقرار"""
        if side == "BUY":
            self.buy_votes += weight
        elif side == "SELL":
            self.sell_votes += weight
        
        if reason:
            log_i(f"🧠 DECISION VOTE: {side} ({weight:.1f}) - {reason}")
    
    def add_block(self, reason: str):
        """إضافة سبب لمنع التداول"""
        self.block_reasons.append(reason)
        log_w(f"🚫 DECISION BLOCK: {reason}")
    
    def get_decision(self, threshold: float = 2.0) -> Dict[str, Any]:
        """الحصول على القرار النهائي"""
        if self.block_reasons:
            return {
                "action": "BLOCK",
                "reason": " | ".join(self.block_reasons),
                "buy_votes": self.buy_votes,
                "sell_votes": self.sell_votes
            }
        
        if self.buy_votes >= threshold and self.buy_votes > self.sell_votes:
            return {
                "action": "BUY",
                "confidence": min(self.buy_votes / (threshold * 2), 1.0),
                "buy_votes": self.buy_votes,
                "sell_votes": self.sell_votes
            }
        
        if self.sell_votes >= threshold and self.sell_votes > self.buy_votes:
            return {
                "action": "SELL",
                "confidence": min(self.sell_votes / (threshold * 2), 1.0),
                "buy_votes": self.buy_votes,
                "sell_votes": self.sell_votes
            }
        ...
        return {
            "action": "WAIT",
            "reason": f"No clear signal (BUY: {self.buy_votes:.1f}, SELL: {self.sell_votes:.1f})",
            "buy_votes": self.buy_votes,
            "sell_votes": self.sell_votes
        }
...
DECISION_MATRIX_THRESHOLD = 2.0

# Bot Version
BOT_VERSION = f"ULTRA PRO AI v13.0 - SMART MONEY EDITION - {EXCHANGE_NAME.upper()} - AUTO-RECOVERY ENABLED"
...
class ColorLogger:
    ...
    @staticmethod
    def smart_money(msg: str):
        print(colored(msg, "blue", attrs=["bold"]))

    @staticmethod
    def decision(msg: str):
...
log_sm = ColorLogger.smart_money
log_dec = ColorLogger.decision
log_blk = ColorLogger.block
...
# ============================================
#  SMART MONEY PANEL SYSTEM
# ============================================

def log_smart_money_panel(smart_money_analysis: dict, trend_state: str, decision_matrix: dict):
    """لوج Smart Money المتكامل"""
    
    if not SMART_MONEY_ENABLED:
        return
    
    sm = smart_money_analysis or {}
    
    # Market Regime
    market_state = sm.get("market_state", {})
    if isinstance(market_state, dict):
        regime = market_state.get("regime", "UNKNOWN")
        trend_str = market_state.get("trend_strength", 0)
        direction = market_state.get("direction", "NONE")
    else:
        regime = "UNKNOWN"
        trend_str = 0
        direction = "NONE"
    ...
    # Liquidity
    liq_state = sm.get("liquidity_state", {})
    if isinstance(liq_state, dict):
        swept_high = liq_state.get("swept_high", False)
        swept_low = liq_state.get("swept_low", False)
    else:
        swept_high = swept_low = False
    
    # Structure
    struct_state = sm.get("structure_state", {})
    if isinstance(struct_state, dict):
        bos = struct_state.get("bos", False)
        choch = struct_state.get("choch", False)
        struct_dir = struct_state.get("direction", "NONE")
    else:
        bos = choch = False
        struct_dir = "NONE"
    
    # Order Block & FVG
    ob = sm.get("order_block", {})
    fvg = sm.get("fvg", {})
    price_in_ob = sm.get("price_in_ob", False)
    price_in_fvg = sm.get("price_in_fvg", False)
    
    # Liquidity Snapshot
    liq_snapshot = sm.get("liquidity_snapshot", "…")
    
    # Fake Breakout
    fake_flags = sm.get("fake_breakout_flags", [])
    
    # Explosion/Collapse
    explosion = sm.get("explosion_state", "NORMAL")
    
    log_sm("=" * 80)
    log_sm("🧠 SMART MONEY HYBRID ENGINE")
    log_sm("=" * 80)
    
    # Market Regime
    regime_color = "🟢" if regime == "TREND" else "🟡" if regime == "RANGE" else "🔴"
    log_sm(f"{regime_color} MARKET: {regime} | Strength: {trend_str:.2f} | Dir: {direction}")
    
    # Trend Classification
    trend_color = "🟢" if trend_state == "LARGE" else "🟡" if trend_state == "MID" else "🔴"
    log_sm(f"{trend_color} TREND: {trend_state}")
    ...
    log_sm(f"💧 LIQUIDITY: {liq_icons} | Snapshot: {liq_snapshot}")
    ...
    log_sm(f"🏗️ STRUCTURE: {struct_icons} | Dir: {struct_dir}")
    
    # Order Block & FVG
    ob_status = "✅" if price_in_ob else "❌"
    fvg_status = "✅" if price_in_fvg else "❌"
    
    log_sm(f"🧱 ZONES: OB={ob_status} | FVG={fvg_status}")
    
    # Explosion/Collapse
    if explosion != "NORMAL":
        exp_color = "💥" if "EXPLOSION" in explosion else "🩸"
        log_sm(f"{exp_color} MOVE: {explosion}")
    
    # Fake Breakout
    if fake_flags:
        log_sm(f"🚫 FAKE: {', '.join(fake_flags)}")
    
    # Decision Matrix
    action = decision_matrix.get("action", "WAIT")
    confidence = decision_matrix.get("confidence", 0.0)
    
    if action == "BUY":
        action_icon = "🟢"
        log_dec(f"{action_icon} DECISION: {action} | Confidence: {confidence:.2f} | Votes: BUY={decision_matrix.get('buy_votes', 0):.1f}, SELL={decision_matrix.get('sell_votes', 0):.1f}")
    elif action == "SELL":
        action_icon = "🔴"
        log_dec(f"{action_icon} DECISION: {action} | Confidence: {confidence:.2f} | Votes: BUY={decision_matrix.get('buy_votes', 0):.1f}, SELL={decision_matrix.get('sell_votes', 0):.1f}")
    elif action == "BLOCK":
        action_icon = "🚫"
        log_blk(f"{action_icon} DECISION: {action} | Reason: {decision_matrix.get('reason', 'Unknown')}")
    else:
        action_icon = "⏳"
        log_sm(f"{action_icon} DECISION: {action} | {decision_matrix.get('reason', 'Waiting...')}")
...
def log_banner():
    ...
    print(colored("  • SMART MONEY HYBRID ENGINE - SMC + Liquidity + Structure", "yellow"))
    print(colored("  • DECISION MATRIX ENGINE - نظام تصويت ذكي", "yellow"))
    print(colored("  • SMART TRAILING & TP LADDER - جني أرباح ذكي حسب نوع الترند", "yellow"))
...
class StateManager:
    ...
    def __init__(self):
        self.state = {
            ...
            "smart_money_tp_ladder": [],
            "trend_state": "CHOP",
            "smart_trailing_sl": None
        }
...
class SmartPositionManager:
    ...
    def __init__(self, exchange_manager, state_manager):
        ...
        self.smart_trailing_engine = None
        self.tp_ladder_engine = None
    ...
    def manage_position(self, df):
        ...
        if not closed:
            ...
            if current_price:
                ...
                # استخدام Smart Trailing Engine إذا كان ممكنا
                if SMART_MONEY_ENABLED and self.state.get("trend_state") in ["MID", "LARGE"]:
                    candles = df.to_dict('records')
                    trailing_side = "BUY" if side == "long" else "SELL"
                    self.smart_trailing_engine = SmartTrailingEngine(candles, trailing_side)
                    
                    current_sl = self.profit_engine.sl_price
                    if current_sl:
                        new_sl = self.smart_trailing_engine.trailing_stop(entry_price, current_sl)
                        
                        if new_sl != current_sl:
                            self.profit_engine.sl_price = new_sl
                            self.state["smart_trailing_sl"] = new_sl
                            log_i(f"🧠 SMART TRAILING UPDATE: SL moved from {current_sl:.6f} to {new_sl:.6f}")
...
class UltraProAIBot:
    ...
    def __init__(self):
        ...
        # Smart Money Engine Components
        self.smart_money_engine = None
        self.trend_classifier = None
        self.tp_ladder_engine = None
        self.decision_matrix_engine = None
        
        self.running = False
        
    def start(self):
        """بدء تشغيل البوت"""
        log_g("🚀 Starting ULTRA PRO AI Trading Bot - SMART MONEY EDITION...")
        ...
        log_g("🔹 NEW: SMART MONEY HYBRID ENGINE + DECISION MATRIX + SMART TRAILING + TP LADDER")
    ...
    def _handle_trading_decision(self, df, current_price, balance):
        """معالجة قرار التداول المتكامل مع Smart Money Engine"""
        if balance <= 10:
            return
        
        # ============================================
        # SMART MONEY ENGINE ANALYSIS
        # ============================================
        smart_money_analysis = {}
        trend_state = "CHOP"
        decision_matrix_result = {"action": "WAIT", "reason": "Initializing..."}
        
        if SMART_MONEY_ENABLED:
            try:
                # تحويل DataFrame إلى قائمة من القواميس لـ Smart Money Engine
                candles = df.to_dict('records')
                
                # 1. Trend Classifier
                trend_info = compute_indicators(df)
                adx = trend_info.get("adx", 0)
                di_plus = trend_info.get("plus_di", 0)
                di_minus = trend_info.get("minus_di", 0)
                ...
                trend_state = self.trend_classifier.classify()
                self.state["trend_state"] = trend_state
                
                # 2. Smart Money Hybrid Engine
                self.smart_money_engine = SmartMoneyHybridEngine(candles)
                
                # جمع كل التحليلات
                atr_value = trend_info.get("atr", current_price * 0.01)
                
                smart_money_analysis = {
                    "market_state": self.smart_money_engine.market_state(),
                    "liquidity_state": self.smart_money_engine.liquidity_state(),
                    "structure_state": self.smart_money_engine.structure_state(),
                    "explosion_state": self.smart_money_engine.explosion_state(atr_value),
                    "order_block": self.smart_money_engine.detect_order_block(),
                    "fvg": self.smart_money_engine.detect_fvg(),
                    "price_in_ob": False,
                    "price_in_fvg": False,
                    "liquidity_snapshot": self.smart_money_engine.liquidity_snapshot(),
                    "fake_breakout_flags": self.smart_money_engine.detect_fake_breakout(atr_value)
                }
                
                # تحقق من وجود السعر في Order Block أو FVG
                ob = smart_money_analysis["order_block"]
                fvg = smart_money_analysis["fvg"]
                
                if ob:
                    smart_money_analysis["price_in_ob"] = self.smart_money_engine.price_in_zone(ob)
                
                if fvg:
                    smart_money_analysis["price_in_fvg"] = self.smart_money_engine.price_in_zone(fvg)
                
                # 3. تحليل Council التقليدي
                decision, reason, council_analysis = self.council.should_enter_trade(df)
                
                # SMC signal من Council
                smc_signal = None
                if decision:
                    smc_signal = decision.upper()
                
                # 4. Decision Matrix Engine
                self.decision_matrix_engine = DecisionMatrixEngine()
                
                # HARD FILTERS - أولوية قصوى
                market_state = smart_money_analysis.get("market_state", MarketState("UNKNOWN", 0, "NONE"))
                if market_state.regime == "NO_TRADE":
                    self.decision_matrix_engine.add_block(f"Market Regime: {market_state.regime}")
                
                fake_flags = smart_money_analysis.get("fake_breakout_flags", [])
                if fake_flags:
                    self.decision_matrix_engine.add_block(f"Fake Breakout: {', '.join(fake_flags)}")
                
                # Trend Classification Filter
                if trend_state == "CHOP":
                    self.decision_matrix_engine.add_block("Market in CHOP - No Trade")
                
                # VOTES - إضافة تصويتات ذكية
                # 1. Council Vote
                if smc_signal:
                    self.decision_matrix_engine.add_vote(smc_signal, weight=2.0, reason="Council AI Signal")
                
                # 2. Liquidity Vote
                liq_state = smart_money_analysis.get("liquidity_state", LiquidityState(False, False, None))
                if liq_state.swept_low:
                    self.decision_matrix_engine.add_vote("BUY", weight=1.5, reason="Liquidity Sweep Low")
                if liq_state.swept_high:
                    self.decision_matrix_engine.add_vote("SELL", weight=1.5, reason="Liquidity Sweep High")
                
                # 3. Structure Vote
                struct_state = smart_money_analysis.get("structure_state", StructureState(False, False, "NONE"))
                if struct_state.direction == "BULL":
                    self.decision_matrix_engine.add_vote("BUY", weight=1.0, reason="Bull Structure")
                elif struct_state.direction == "BEAR":
                    self.decision_matrix_engine.add_vote("SELL", weight=1.0, reason="Bear Structure")
                
                # 4. Order Block / FVG Vote
                if smart_money_analysis.get("price_in_ob"):
                    ob_type = smart_money_analysis["order_block"].type if smart_money_analysis["order_block"] else None
                    if ob_type == "BUY":
                        self.decision_matrix_engine.add_vote("BUY", weight=1.5, reason="In Buy Order Block")
                    elif ob_type == "SELL":
                        self.decision_matrix_engine.add_vote("SELL", weight=1.5, reason="In Sell Order Block")
                
                if smart_money_analysis.get("price_in_fvg"):
                    self.decision_matrix_engine.add_vote("BUY" if smart_money_analysis.get("price_in_ob") else "SELL", 
                                                         weight=1.0, reason="In FVG Zone")
                
                # 5. Explosion Vote
                explosion = smart_money_analysis.get("explosion_state", "NORMAL")
                if explosion == "EXPLOSION_UP":
                    self.decision_matrix_engine.add_vote("BUY", weight=1.0, reason="Explosion UP")
                elif explosion == "EXPLOSION_DOWN":
                    self.decision_matrix_engine.add_vote("SELL", weight=1.0, reason="Explosion DOWN")
                
                # الحصول على القرار النهائي
                decision_matrix_result = self.decision_matrix_engine.get_decision(threshold=DECISION_MATRIX_THRESHOLD)
                
                # تسجيل Smart Money Panel
                log_smart_money_panel(smart_money_analysis, trend_state, decision_matrix_result)
                
            except Exception as e:
                log_e(f"❌ Smart Money Engine error: {e}")
                traceback.print_exc()
                decision_matrix_result = {"action": "WAIT", "reason": f"SM Engine Error: {e}"}
        
        # ============================================
        # TRADITIONAL COUNCIL ANALYSIS
        # ============================================
        ...
        if SMART_MONEY_ENABLED:
            # استخدام قرار Smart Money كأولوية
            sm_action = decision_matrix_result.get("action")
            
            if sm_action == "BUY":
                final_decision = "BUY"
                final_reason = f"SMART MONEY DECISION: {decision_matrix_result.get('confidence', 0):.2f} confidence"
            elif sm_action == "SELL":
                final_decision = "SELL"
                final_reason = f"SMART MONEY DECISION: {decision_matrix_result.get('confidence', 0):.2f} confidence"
            elif sm_action == "BLOCK":
                log_blk(f"🚫 SMART MONEY BLOCKED: {decision_matrix_result.get('reason', 'Unknown')}")
                return
            else:
                # إذا كان Smart Money ينتظر، نستخدم قرار Council
                if decision:
                    final_decision = decision.upper()
                    final_reason = f"COUNCIL DECISION: {reason}"
        else:
            # استخدام قرار Council مباشرة
            if decision:
                final_decision = decision.upper()
                final_reason = f"COUNCIL DECISION: {reason}"
        
        if final_decision:
            log_dec(f"🎯 FINAL DECISION: {final_decision} | {final_reason}")
            ...
            # إعداد TP Ladder إذا كان ترند مناسب
            if trend_state in ["MID", "LARGE"]:
                candles = df.to_dict('records')
                self.tp_ladder_engine = TPLadderEngine(
                    candles=candles,
                    side=final_decision,
                    trend_type=trend_state
                )
                
                tp_ladder = self.tp_ladder_engine.ladder(current_price)
                if tp_ladder:
                    log_i(f"📈 SMART TP LADDER: {[(tp['tp'], f'{tp['close_pct']*100:.0f}%') for tp in tp_ladder]}")
                    self.state["smart_money_tp_ladder"] = tp_ladder

            # فتح المركز
            if self.position_manager.open_position(final_decision, df, analysis):
                log_g(f"💰 Position opened successfully | Type: {'SMART MONEY' if SMART_MONEY_ENABLED else 'COUNCIL'} | Signals: {len(analysis['signals'])}")
                ...
                log_e("❌ Failed to open position")
        else:
            if SMART_MONEY_ENABLED:
                if decision_matrix_result.get("action") == "WAIT":
                    log_i(f"⏳ SMART MONEY WAITING: {decision_matrix_result.get('reason', 'No confluence')}")
            elif analysis.get("confidence", 0) > 0.3:
                log_i(f"⏳ COUNCIL WAITING: {reason}")
    ...
    def get_status(self):
        ...
        status = {
            ...
            "trend_state": self.state.get("trend_state", "UNKNOWN")
        }
        ...
        # إضافة معلومات Smart Money TP Ladder
        if self.state.get("smart_money_tp_ladder"):
            status["smart_money_tp_ladder"] = self.state["smart_money_tp_ladder"]
...
def create_app(bot_instance):
    ...
    @app.route("/")
    def home():
        return "OK - ULTRA PRO AI BOT LIVE - SMART MONEY EDITION"
    ...
    def metrics():
        ...
        return jsonify({
            ...
            "trend_state": status.get("trend_state", "UNKNOWN"),
            "smart_money_enabled": status.get("smart_money_enabled", False),
    ...
    @app.route("/toggle_smart_money")
    def toggle_smart_money():
        global SMART_MONEY_ENABLED
        SMART_MONEY_ENABLED = not SMART_MONEY_ENABLED
        return jsonify({
            "status": "ok",
            "smart_money_enabled": SMART_MONEY_ENABLED,
            "message": f"Smart Money Engine {'enabled' if SMART_MONEY_ENABLED else 'disabled'}"
        })
